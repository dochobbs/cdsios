...
final class SecureSettings: ObservableObject {
    ...
    @Published private(set) var model: OllamaConfiguration.Model = .default

    private let keychainService = "com.dochobbs.clinicalapp"
    private let apiKeyKey = "ollama_api_key"
    ...
    private init() {
        ...
        if let storedModel = UserDefaults.standard.string(forKey: modelKey),
           let model = OllamaConfiguration.Model(rawValue: storedModel) {
            self.model = model
        }
    }

    func update(apiKey: String, model: OllamaConfiguration.Model) throws {
    ...
    func configuration() -> OllamaConfiguration {
        OllamaConfiguration(apiKey: apiKey, model: model)
    }
