...
- `ClinicalApp.xcodeproj` – iOS 17 SwiftUI project.
- `ClinicalApp/` – Source files, assets, prompts, and Info.plist.
- `ClinicalApp/Resources/Prompts` – JSON prompt exports generated via `python scripts/export_prompts.py`.

### Getting Started

1. Run `python scripts/export_prompts.py` at the repo root whenever prompts change.
2. Open `ios/ClinicalApp/ClinicalApp.xcodeproj` in Xcode 15+.
...
| File | Purpose |
| --- | --- |
| `ClinicalAppApp.swift` | Entry point with tab navigation + Settings controls |
...
4. Register the tab or screen in `ClinicalAppApp` so it shows up beside Drug Lookup and CDS.
